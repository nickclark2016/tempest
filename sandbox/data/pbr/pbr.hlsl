...
float3 normal_from_map(uint tex_id, SamplerState s, float2 uv, float3 world_pos, float4 tangent, float3 world_normal)
{
    if (tex_id == MAX_UINT)
    {
        return world_normal;
    ...
    return normalize(mul(tan_normal, tbn));
}
...
PSInput VSMain(uint index_id : SV_VertexID, uint instance_id : SV_InstanceID)
{
    ...
    result.normal = mul(model_3x3, v.normal);
    result.tangent = float4(mul(model_3x3, v.tangent.xyz), v.tangent.w);
...
float4 PSMain(PSInput input) : SV_Target
{
    ...
    Texture2D base_texture = bindless_textures[mat.base_texture];
    
    float4 base_alpha = base_texture.Sample(bindless_sampler, input.uv);
    ...
    float3 normal = normal_from_map(mat.normal, bindless_sampler, input.uv, input.world_pos, float4(1, 0, 0, 0), input.normal);
