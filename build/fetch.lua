...
newaction {
    ...
    execute = function()
        ...
        if _TARGET_OS == 'linux' then
            local slangZipPath = path.join(cacheDir, 'slang-2024.14.6-linux-64.zip')
            local downloaded = fetchIfNotExists('https://github.com/shader-slang/slang/releases/download/v2024.14.6/slang-2024.14.6-linux-x86_64.zip', slangZipPath)
            if downloaded then
                zip.extract(slangZipPath, path.join(_MAIN_SCRIPT_DIR, 'dependencies/slang'))
            end
        end
