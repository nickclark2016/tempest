...
namespace tempest::render_system
{
                                       ...
                                       const core::texture_registry& textures, const core::material_registry& materials)
    {
        ...
        for (const auto entity : entities)
        {
            ...
            const auto obj_idx = static_cast<uint32_t>(objects.size());
            objects.push_back(object_payload{
                .model = model_mat,
                                ...
                                optional<rhi::texture_view_handle> swapchain_view)
    {
        ...
        auto scene = scene_constants{
            ...
            .ambient_light = {0.05F, 0.05F, 0.08F, 1.0F},
        ...
        if (_camera_system)
        {
            if (auto cam_opt = _camera_system->get_active_camera())
            {
        ...
        {
            _inputs.entity_registry->each([&scene](const directional_light_component& dl,
