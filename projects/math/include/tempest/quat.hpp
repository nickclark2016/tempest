...
{
    template <typename T> struct alignas(sizeof(T) * 4) quat
    {
        union {
            T data[4];
            struct
    ...
    template <typename T> quat(const T) -> quat<T>;

    template <typename T> quat(const T, const T, const T, const T) -> quat<T>;

    template <typename T> quat(const vec3<T>&) -> quat<T>;

    template <typename T> quat(const quat<T>&) -> quat<T>;

    template <typename T> quat(quat<T>&&) -> quat<T>;

    template <typename T> inline constexpr quat<T>::quat() : quat(T(1), T(0), T(0), T(0))
    {
    }

    template <typename T> inline constexpr quat<T>::quat(const T scalar) : quat(scalar, scalar, scalar, scalar)
    {
    }
    ...
    template <typename T> inline constexpr quat<T>::quat(const vec3<T>& euler)
    {
    ...
    template <typename T> inline constexpr T& quat<T>::operator[](const std::size_t index) noexcept
    {
        return data[index];
    }

    template <typename T> inline constexpr const T& quat<T>::operator[](const std::size_t index) const noexcept
    {
        return data[index];
    }

    template <typename T> inline constexpr quat<T> operator*(const quat<T>& lhs, const quat<T>& rhs)
    {
    ...
    template <typename T> inline constexpr quat<T> operator*(const T lhs, const quat<T>& rhs)
    {
        return quat(lhs * rhs.w, lhs * rhs.x, lhs * rhs.y, lhs * rhs.z);
    }

    template <typename T> inline constexpr quat<T> operator-(const quat<T>& q)
    {
        return quat(-q.w, -q.x, -q.y, -q.z);
    }

    template <typename T> inline constexpr T norm(const quat<T>& q)
    {
