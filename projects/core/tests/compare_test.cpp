...
TEST(test_compare, three_way_compare_floating_points)
{
    ...
    // Test against NaN
    ASSERT_EQ(tempest::compare_three_way{}(1.0f, std::numeric_limits<float>::quiet_NaN()),
              tempest::strong_ordering::less);

    ASSERT_EQ(tempest::compare_three_way{}(std::numeric_limits<float>::quiet_NaN(), 1.0f),
              tempest::strong_ordering::greater);

    ASSERT_EQ(tempest::compare_three_way{}(std::numeric_limits<float>::quiet_NaN(),
