...
{
    unique_ptr<render_context> render_context::create(abstract_allocator* alloc)
    {
        return make_unique<vk::render_context>(alloc);
    }
    ...
    namespace
    {
        resource_format convert(core::texture_format fmt)
        {
            switch (fmt)
            {
                                                                      ...
                                                                      bool use_entire_buffer, bool generate_mip_maps)
    {
        ...
        {
            const auto& tex_dat = tex_reg.get_texture(tex_guid).value();
            auto mip_count = generate_mip_maps
                                 ? std::bit_width(std::min(tex_dat.mips[0].width, tex_dat.mips[0].height)) - 1
                                 : static_cast<std::uint32_t>(tex_dat.mips.size());
        ...
        for (const auto tex_guid : texture_ids)
        {
            const auto& tex_data = tex_reg.get_texture(tex_guid).value();

            std::uint32_t mip_index = 0;

            for (const auto& mip_data : tex_data.mips)
            {
                size_t mip_size = mip_data.data.size();
                size_t mip_bytes_written = 0;
                size_t row_width_in_bytes = mip_data.width * bytes_per_element(convert(tex_data.format));
                size_t row_index = 0;

                while (mip_bytes_written < mip_size)
                {
                    size_t bytes_available = staging_buffer_size - staging_buffer_bytes_written;
                    size_t bytes_to_write = std::min(mip_size - mip_bytes_written, bytes_available);
                    size_t row_count = bytes_to_write / row_width_in_bytes;
        ...
        if (generate_mip_maps)
        {
            for (std::size_t i = 0; i < images.size(); ++i)
            {
                auto guid = texture_ids[i];
                auto& tex_dat = tex_reg.get_texture(guid).value();
                auto handle = images[i];
                                                          ...
                                                          buffer_resource_handle staging_buffer)
    {
        ...
        {
            optional<core::mesh&> m = mesh_reg.find(mesh_id);
            auto& mesh = m.value();
            if (!mesh.has_tangents)
            {
                mesh.compute_tangents();
