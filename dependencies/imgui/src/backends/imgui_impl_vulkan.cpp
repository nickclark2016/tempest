...
//              *BREAKING CHANGE*: Removed ImGui_ImplVulkan_DestroyFontUploadObjects() which is now unecessary as we
...
static void CreateOrResizeBuffer(VkBuffer& buffer, VmaAllocation& alloc, VmaAllocator allocator,
                                 VkDeviceSize& p_buffer_size, size_t new_size, VkBufferUsageFlagBits usage)
{
    ...
    err = vmaCreateBuffer(allocator, &buffer_info, &ci, &buffer, &alloc, &alloc_info);
    check_vk_result(err);
    p_buffer_size = alloc_info.size;
}
...
void ImGui_ImplVulkan_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    if (draw_data->TotalVtxCount > 0)
    {
        ...
        if (rb->VertexBuffer == VK_NULL_HANDLE || rb->VertexBufferSize < vertex_size)
            CreateOrResizeBuffer(rb->VertexBuffer, rb->VertexBufferAllocation, v->MemoryAllocator, rb->VertexBufferSize,
                                 vertex_size, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
        if (rb->IndexBuffer == VK_NULL_HANDLE || rb->IndexBufferSize < index_size)
            CreateOrResizeBuffer(rb->IndexBuffer, rb->IndexBufferAllocation, v->MemoryAllocator, rb->IndexBufferSize, index_size,
        ...
        VkResult err = vmaMapMemory(v->MemoryAllocator, rb->VertexBufferAllocation, (void**)(&vtx_dst));
        check_vk_result(err);
        err = vmaMapMemory(v->MemoryAllocator, rb->IndexBufferAllocation, (void**)(&idx_dst));
        check_vk_result(err);
        ...
        const VmaAllocation allocs_to_flush[] = {rb->VertexBufferAllocation, rb->IndexBufferAllocation};
        const VkDeviceSize sizes_to_flush[] = {rb->VertexBufferSize, rb->IndexBufferSize};
        const VkDeviceSize offsets_to_flush[] = {0, 0};

        vmaFlushAllocations(v->MemoryAllocator, 2, allocs_to_flush, offsets_to_flush, sizes_to_flush);

        vmaUnmapMemory(v->MemoryAllocator, rb->VertexBufferAllocation);
        vmaUnmapMemory(v->MemoryAllocator, rb->IndexBufferAllocation);
    }
...
bool ImGui_ImplVulkan_CreateFontsTexture()
{
    ...
    if (bd->FontView || bd->FontImage || bd->FontMemoryAllocation || bd->FontDescriptorSet)
    {
    ...
    // Create the Image:
    {
        ...
        err = vmaCreateImage(v->MemoryAllocator, &info, &alloc_create_info, &bd->FontImage, &bd->FontMemoryAllocation, nullptr); 
        check_vk_result(err);
    }
    ...
    VmaAllocation upload_buffer_alloc;
    VkBuffer upload_buffer;
    {
        ...
        err = vmaCreateBuffer(v->MemoryAllocator, &buffer_info, &alloc_ci, &upload_buffer, &upload_buffer_alloc, nullptr);
        check_vk_result(err);
    }
    ...
    {
        char* map = nullptr;
        err = vmaMapMemory(v->MemoryAllocator, upload_buffer_alloc, (void**)(&map));

        check_vk_result(err);
        memcpy(map, pixels, upload_size);

        err = vmaFlushAllocation(v->MemoryAllocator, upload_buffer_alloc, 0, upload_size);
        
        check_vk_result(err);

        vmaUnmapMemory(v->MemoryAllocator, upload_buffer_alloc);
    }
    ...
    vmaDestroyBuffer(v->MemoryAllocator, upload_buffer, upload_buffer_alloc);
...
void ImGui_ImplVulkan_DestroyFontsTexture()
{
    ...
    if (bd->FontMemoryAllocation)
    {
        vmaDestroyImage(v->MemoryAllocator, bd->FontImage, bd->FontMemoryAllocation);
    }
}
...
void ImGui_ImplVulkan_DestroyDeviceObjects()
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    ImGui_ImplVulkanH_DestroyWindowRenderBuffers(v->Device, &bd->MainWindowRenderBuffers, v->MemoryAllocator);
    ImGui_ImplVulkan_DestroyFontsTexture();
...
#undef IMGUI_VULKAN_FUNC_LOAD
    ...
    ImGuiImplVulkanFuncs_vkCmdBeginRendering =
        reinterpret_cast<PFN_vkCmdBeginRendering>(loader_func("vkCmdBeginRendering", user_data));
    ImGuiImplVulkanFuncs_vkCmdEndRendering =
        reinterpret_cast<PFN_vkCmdEndRendering>(loader_func("vkCmdEndRendering", user_data));
#endif
#else
...
bool ImGui_ImplVulkan_Init(ImGui_ImplVulkan_InitInfo* info, VkRenderPass render_pass)
{
    ...
    if (!bd->FontSampler)
    {
        ...
        err = vkCreateSampler(v->Device, &sampler_info, v->Allocator, &bd->FontSampler);
        check_vk_result(err);
    }
    ...
    if (!bd->DescriptorSetLayout)
    {
        ...
        err = vkCreateDescriptorSetLayout(v->Device, &set_layout_info, v->Allocator, &bd->DescriptorSetLayout);
        check_vk_result(err);
    }
                                                 ...
                                                 VmaAllocator allocator)
{
    ...
    {
        vmaDestroyBuffer(allocator, buffers->VertexBuffer, buffers->VertexBufferAllocation);
    }
    if (buffers->IndexBuffer)
    {
        vmaDestroyBuffer(allocator, buffers->IndexBuffer, buffers->IndexBufferAllocation);
    }
